...
def preprocess_qwen_2_visual(
    ...
    for i, source in enumerate(sources):
        ...
        ###################### PersonaVLM  ##################################
        # comptible with the case where the last round is not trained
        train_label = [1] * int(len(source) / 2)
        train_label[-1] = 0
        ################################################################
        ...
        for conv in source:
            ...
            if role in ["user", "system"]:
                ...
                ###################### PersonaVLM ##################################
                if train_label[0] == 1:
                    target_mask[:3] = [IGNORE_INDEX] * 3 # 151644, 77091（or 872）, 198（换行）,
                else:
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            # print(data_dict['input_ids'].shape, data_dict['labels'])
            # assert 1==2
