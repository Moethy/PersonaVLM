...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def main_thread_print(self, content):
        from torch import distributed as dist
        local_rank = int(dist.get_rank()) if dist.is_initialized() else 0
        if local_rank == 0:
    ...
    def _generate_completions(self, inputs: DataType) -> DataType:
        ...
        inputs = self._preprocess_inputs(inputs)
        # self.main_thread_print(inputs)

        mode = 'train' if self.model.training else 'eval'
        if self.use_fast_infer:
            results = self._fast_infer(inputs)
            # self.main_thread_print(results)
            # assert 1==2
        else:
    ...
    def _generate_and_score_completions(self, inputs: DataType) -> DataType:
        ...
        if self.template.truncation_strategy == 'raise':
            inputs = self.resample_encode_failed_inputs(inputs)
        self.ori_input = deepcopy(inputs)
        ...
        for i, (reward_func, reward_model_plugin, reward_func_name) in enumerate(
            ...
            with patch_profiling_context(self, reward_func_name):
                ...
                # reward function
                    ...
                    output_reward_func = reward_func(completions, self.ori_input, **reward_kwargs)
