...
class UserConfig:
    ...
    # These must be filled for the application to run.
    USER_ID = ""
    DATA_STORAGE_PATH = "./data"
    # --- Optional Configurations ---
    # These provide additional context but are not mandatory.
    PROFILE = {}
...
def update_personality(data):
    ...
    for session in data["sessions"][:-1]:
        for idx in range(len(session["interactions"])):
            infer_personslity = session["big_five_personality"][idx][0]
            EMA_moving_factor = cosine_decay_factor(current_step)
            # print(EMA_moving_factor)
            update_personality = {
                "openness": EMA_moving_factor * personality["openness"] + (1 - EMA_moving_factor) * infer_personslity["openness"]["score"],
                "conscientiousness": EMA_moving_factor * personality["conscientiousness"] + (1 - EMA_moving_factor) * infer_personslity["conscientiousness"]["score"],
                "extraversion": EMA_moving_factor * personality["extraversion"] + (1 - EMA_moving_factor) * infer_personslity["extraversion"]["score"],
                "agreeableness": EMA_moving_factor * personality["agreeableness"] + (1 - EMA_moving_factor) * infer_personslity["agreeableness"]["score"],
                "neuroticism": EMA_moving_factor * personality["neuroticism"] + (1 - EMA_moving_factor) * infer_personslity["neuroticism"]["score"],
            }
            personality = copy.deepcopy(update_personality)
            # print(session["big_five_personality"][idx][1], update_personality)
            session["big_five_personality"][idx][1] = copy.deepcopy(update_personality)
            current_step += 1
    data["big_five_personality"] = copy.deepcopy(update_personality)
...
# get all users
import sys, json, os
sys.path.append("./PersonaVLM")
import PersonaVLMAgent
import prompts

ROOT_DIR = "./data/training/rl/samples"
user_path_dict_path = "./data/training/rl/USER_PATH_DICT.json"
with open(user_path_dict_path, 'r', encoding='utf-8') as f:
    USER_PATH_DICT = json.load(f)
print(len(USER_PATH_DICT))

class MultiTurnSchedulerOurs(MultiTurnScheduler):
    def __init__(self, *args, **kwargs):
        from .orm import MathAccuracy
        super().__init__(*args, **kwargs)
        self.retrieve_manager = {}
    def step(self, infer_request: 'RolloutInferRequest', response_choice: 'ChatCompletionResponseChoice',
             current_turn: int) -> Dict:
        # print(12345678910, infer_request)
        uuid = infer_request.uuid
        input_round_0 = infer_request.messages[1]['content']
        cur_output = infer_request.messages[-1]['content']
        if '''</answer>''' in cur_output and \
            not ('\nOptions:\n' in str(infer_request.messages) and "Retrieved Procedural Memory" not in str(infer_request.messages)):
            if uuid in self.retrieve_manager:
                self.retrieve_manager[uuid].retriever.reset_index()
            return infer_request
        # retrieve
        # 判断是否需要构建新的agent
        content = input_round_0.split('\n\nUser Input:\n')[1].split('\n\n# Core Instructions\n')[0].strip()
        time = content.split("\'time\': \'")[1].split('(')[0].strip()
        query = content.split("\'query\': ")[1].strip().strip('}').strip('\"').strip('\'').strip()
        try:
            retrieve_info = {}
            if "</answer>" not in cur_output:
                retrieve_info["keywords"] = query
                retrieve_info["start_time"] = retrieve_info["end_time"] = None
                retrieve_info["retrieve"] = True
            else:
                retrieve_info["keywords"] = cur_output.split('"keywords":')[1].split('"start_time":')[0].strip().strip('"')
                start_time = cur_output.split('"start_time":')[1].split('"end_time"')[0].strip().strip('"')
                end_time = cur_output.split('"end_time":')[1].split('</retrieve>')[0].strip().strip('"')
            ...
            retrieve_info = {"keywords": query, "start_time": None, "end_time": None, "retrieve": True}
        # gogogo 检索具体内容
        if uuid in self.retrieve_manager:
            agent = self.retrieve_manager[uuid]
        else:
            # print(USER_PATH_DICT[input_round_0])
            data_path = USER_PATH_DICT[input_round_0].replace("training/data/persona_trainV8_100", "DatasetPipeline/data/persona_trainV5_100")
            # get data file
            data_path = os.path.join(data_path, [f for f in os.listdir(data_path) if f.endswith(".json") and f.split('.')[0].isdigit()][0])
            # print(data_path, USER_PATH_DICT[input_round_0], 3333)

            with open(data_path, 'r') as f:
                data = json.load(f)
            update_personality(data)
            user_config = UserConfig()
            user_config.DATA_STORAGE_PATH = USER_PATH_DICT[input_round_0]
            user_config.PROFILE = data["init_profile"]
            user_config.USER_ID = data["init_profile"]["name"]
            agent = PersonaVLMAgent.PersonaVLM(user_config, data=data)
            
            self.retrieve_manager[uuid] = agent
        # retrieve 注意内容不能重复
        _query = {
            'time': time,
            'query': query,
        }
        retrieve_memory = agent.retriever.retrieve(_query, retrieve_info, agent.data, agent.session_interval_time, reset=False,
                                                   maximum_episodic_memory_num=random.randint(2,5),
                                                   maximum_semantic_memory_num=random.randint(3,6))
        prompt_reason_sample = prompts.MLLM_REASONING_PROMPT_MID
        prompt_reason_sample = prompt_reason_sample.replace("{ProceduralMemory}", "\n".join(retrieve_memory.get("procedural_memory", [])))
        prompt_reason_sample = prompt_reason_sample.replace("{SemanticMemory}", "\n".join(retrieve_memory.get("semantic_memory", [])).replace("<img_fill>", '<image>'))
        dialog_history = "\n".join(retrieve_memory.get("episodic_memory", []))
        prompt_reason_sample = prompt_reason_sample.replace("{DialogHistory}", dialog_history.strip().replace("<img_fill>", '<image>'))
        ...
        infer_request.messages.append({
            "role": "user",
            "content": prompt_reason_sample,
        })
        infer_request.images.extend(reason_sample_imgs)
        if not str(infer_request.messages).count("<image>") == len(infer_request.images):
            print("worse thing happen Invalid infer_request: ", infer_request)
        return {"infer_request": infer_request, "rollout_infos": {"images": copy.deepcopy(infer_request.images)}}
...
multi_turns = {
    ...
    'MultiTurnSchedulerOurs': MultiTurnSchedulerOurs,
}
