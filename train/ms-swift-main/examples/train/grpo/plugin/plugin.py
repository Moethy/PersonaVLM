    ...
    **kwargs,
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(600.0)) as client:
        try:
            ...
            # print(messages, [choice["message"]["content"] for choice in response_data["choices"]])
...
answer_corr_prompt = '''
You are an expert in evaluating question-answering systems. I will provide a question, a generated answer, and a reference answer. Please evaluate the **Correctness** of the generated answer on an integer scale from 0 to 10, following these rules:

- If the question is **closed-ended** (e.g., multiple-choice, yes/no, true/false, or has a single factual answer), assign:
  - **10** if the generated answer matches the reference answer exactly in content,
  - **0** otherwise.

- If the question is **open-ended**, assign a score from 0 to 10 based on:
  1. **Factual accuracy**: Does the answer convey correct information consistent with the reference?
  2. **Content consistency**: Are the key points, intent, and conclusions aligned with the reference?
  3. **Personality alignment**: Does the answer adapt to the user's personality (e.g., tone, role, emotional style, identity) in a way that matches how the reference answer does?

Give a high score (e.g., 8–10) only if all three aspects are well satisfied. Be strict but fair.
...
Return ONLY a single integer between 0 and 10. Do not include any other text, explanation, punctuation, or formatting.

Your Evaluation Result:
'''.strip()

async def eval_corr(query, ans, gt):
    SERVER_IP = "localhost"
    SERVER_PORT = "8080"
    MODEL_NAME = "/root/bayes-gpfs-c9f955b46c074f048c960ec71693fa58/niechang/data1/huggingface_cache/Qwen3-30B-A3B-Instruct-2507"
    
    _input = answer_corr_prompt
    _input = _input.replace("{input_question}", query)
    _input = _input.replace("{generate_answer}", ans)
    _input = _input.replace("{reference_answer}", gt)
    test_messages = [{"role": "user", "content": _input}]
    for i in range(3):
        try:
            result = await llm_openai_api(
...
consistency_prompt= '''
You are an expert in evaluating logical consistency. I will provide a question, a solution process, and a final answer. Please evaluate the **Consistency** between the process and the answer on an integer scale from 0 to 10.

"Consistency" measures the logical integrity connecting the process to the answer. It ensures the final answer is the product of a **genuine reasoning process** that directly engages with the question's content, not a lucky guess or a result of exploiting non-logical shortcuts.

- Assign **10** if the final answer is a direct, logical, and fully supported conclusion of the solution process.
- Assign **0** if the process contradicts the final answer, is irrelevant to it, or the logic is fundamentally broken.
- For scores in between, evaluate the severity of the logical gaps or errors. A higher score means the process more closely and correctly supports the final answer.
...
async def eval_consistency(query, ans, think):
    SERVER_IP = "localhost"
    SERVER_PORT = "8080"
    MODEL_NAME = "/root/bayes-gpfs-c9f955b46c074f048c960ec71693fa58/niechang/data1/huggingface_cache/Qwen3-30B-A3B-Instruct-2507"
    
    _input = consistency_prompt
    _input = _input.replace("{input_question}", query)
    _input = _input.replace("{final_answer}", ans)
    _input = _input.replace("{solution_process}", think)
    test_messages = [{"role": "user", "content": _input}]
    for i in range(3):
        try:
            result = await llm_openai_api(
...
class OurReward(ORM):
    ...
    def __call__(self, completions, inputs, **kwargs) -> List[float]:
        return self.loop.run_until_complete(self.async_call(completions, inputs))

    async def async_call(self, completions, inputs) -> List[float]:
        tasks = []
        for content, sol in zip(completions, inputs):
            tasks.append(self.evaluate_one(content, sol))
        rewards = await asyncio.gather(*tasks)
        return rewards

    async def evaluate_one(self, content, sol) -> float:
        if '</answer>' not in content:
            return 0.0
        pattern = r'^<think>.*?</think><answer>.*?</answer>$'
        match = re.match(pattern, content, re.DOTALL | re.MULTILINE)
        format_reward = 1.0 if match else 0.0
        for _ in range(3):
            try:
                think = content.split('<think>')[-1].split('</think>')[0].strip()
                pred = content.split('<answer>')[-1].split('</answer>')[0].strip()
                gt = sol["messages"][-1]["content"].strip()

                input_round_0 = sol["messages"][0]["content"].strip()
                user_input_str = input_round_0.split('\n\nUser Input:\n')[1].split('\n\n# Core Instructions\n')[0].strip()
                query = user_input_str.split("\'query\': ")[1].strip().strip('}').strip('"').strip("'").strip()

                if gt.lower() in ['(a)', '(b)', '(c)', '(d)'] and pred.lower()[:3] == gt.lower():
                    score_1 = 1.0
                else:
                    score_1 = await eval_corr(query, pred, gt)
                score_2 = await eval_consistency(query, pred, think)
                score = (score_1 * score_2 + format_reward * 0.5)
                print(f"score: {score}, gt: {gt}, pred: {pred}", score_1, score_2, format_reward, content)
                return score
            except Exception as e:
                print(f"Evaluation error: {e}")
        return 0.0

orms['external_our_reward'] = OurReward
